...
function updateVersionThemePanel() {
    ...
    const targetVersion = /name="theme_version" data-value="([^"]*)"/;
    const targetName = /name="theme_name" data-value="([^"]*)"/;
    const themeVersion = `name="theme_version" data-value="${version}"`;
    const themeName = `name="theme_name" data-value="${name}"`;
    return gulp
        .src(`${paths.theme.config}/settings.html`)
        .pipe(replace(anchor, `${version}</span>`))
        .pipe(replace(targetVersion, themeVersion))
        .pipe(replace(targetName, themeName))
        .pipe(gulp.dest(paths.theme.config));
}
...
const start = gulp.series(parallel(series(processSass, minifyCSS), series(minifyJS, concatLibsJs)), watchFiles);
